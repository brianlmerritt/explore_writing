...
write.py          reads data/grid.tsv, writes data/generations.tsv + output_text/*.md
review.py         reads generations + rubric.yaml, writes data/reviews.tsv
```

Everything is append-only and resumable. If `write.py` dies partway through,
...
4. `python write.py` — generates samples, writes `data/generations.tsv`, and saves cleaned text to markdown files in the `output_text/` subfolder (under `REMOTE_FOLDER_PATH` if set, otherwise the local `output_text/` folder).
...
└── output_text/     generated outputs (*.md, written by write.py)
```

When `REMOTE_FOLDER_PATH` is blank the local repo defaults are used with the same subfolder names
